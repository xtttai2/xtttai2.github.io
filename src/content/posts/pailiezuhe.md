...
换句话说，求 $x_1, x_2, \cdots, x_m$ 的正整数解的数量，满足：
- $\sum x_i = n$
- $x_1 \le x_2 \le \cdots \le x_m$

我们可以通过以下两种操作构造出满足要求的集合[^2]：
1. 添加新元素 $1$。
2. 给集合内所有元素加 $1$。

考虑递推，设 $f(n, m)$ 表示将 $n$ 拆分成 $m$ 个正整数（不区分顺序）的和的方案数。两种操作所对应的方案数分别为 $f(n - 1, m - 1)$ 和 $f(n - m, m)$。所以有
$$
...
[^2]: 整数集合的构造方式：任何整数集合都可以通过以下两种操作构造出来：1. 加入新元素 $a$，$a$ 为所求集合允许的元素最小值（比如求正整数集合，则 $a = 1$）；2. 给所有元素加 $1$．而且，操作序列唯一．
